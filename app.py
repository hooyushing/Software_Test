...
class AutoTestDesignEngine:
    ...
    def analyze_and_generate(self, raw_requirement: str) -> dict:
        ...
        Analyze the following software requirement(s) and output ONLY a valid JSON object.
        ...
        try:
            ...
            st.error(f"Failed to generate or parse response: {e}")
            return None
    ...
    def generate_state_diagram_dot(self, transitions: list) -> str:
        """Generates a DOT language string to draw the State Transition Diagram."""
    ...
    def generate_white_box_test_cases(self, transitions: list) -> list:
        """Models paths and converts them into formal test case structures."""
        if not transitions: return []
        G = nx.DiGraph()
        ...
        for start in start_nodes:
            for end in dead_ends:
                if start != end:
                    # Calculate All-Paths coverage to a terminal state
                    for path in nx.all_simple_paths(G, source=start, target=end):
                        ...
                        wb_tests.append({
                            ...
                            "technique": "State Transition (All-Paths)",
                            "input": f"Execute Sequence: {full_path}",
                            "expected_oracle": f"System halts at terminal state: {end}",
                            "type": "White-Box"
                        })
                        tc_counter += 1
        return wb_tests
...
if st.button("Generate Test Design Artifacts", type="primary"):
    if not api_key:
        ...
        with st.spinner("Analyzing requirements and generating artifacts..."):
            ...
            results = engine.analyze_and_generate(final_requirement_text)
            ...
            if results:
                ...
                with col_diagram:
                    ...
                    if dot_source:
                        ...
                        st.write("No distinct states identified.")
                ...
                with col_table:
                    ...
                    if not df_wb_tests.empty:
                        st.dataframe(df_wb_tests, use_container_width=True)
                    else:
                        st.write("No distinct paths identified.")

                st.divider()

                # FR 6.0 - Now merges BOTH tables into one Export
                st.subheader("FR 6.0: Consolidated Artifact Export")
                
                # Combine Black-Box and White-Box into a single DataFrame
                if not df_wb_tests.empty:
