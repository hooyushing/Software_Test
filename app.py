...
class AutoTestDesignEngine:
    def __init__(self, api_key):
        ...
        # [UPDATED]: The prompt now explicitly forces the AI to break paragraphs into atomic rules.
        self.system_prompt = """
        You are an expert QA Engineer and Requirements Analyst following ISTQB and ISO 29119-4 standards.
        
        CRITICAL INSTRUCTION: If the input is a block of text or a paragraph, you MUST break it down into individual, ATOMIC testable rules or constraints. 
        Do NOT summarize the whole paragraph into a single requirement. 
        For example, if the text says "Amount must be > 0 and card must be 16 digits", you must output TWO separate objects in the array.
        ...
        JSON Structure exactly like this:
        {
            "analyzed_requirements": [
                {
                    "requirement_snippet": "The specific atomic rule (e.g., 'The transaction amount must be greater than $0.00')",
                    "parsed_components": ["Input: Amount", "Condition: > 0.00"],
    ...
    def analyze_and_generate(self, raw_requirement: str) -> dict:
        prompt = f"""
        {self.system_prompt}
        Analyze the following text and extract EVERY individual software requirement:
        "{raw_requirement}"
        """
    ...
    def refine_artifacts(self, raw_requirement: str, current_json: dict, user_feedback: str) -> dict:
        prompt = f"""
        {self.system_prompt}
        Original Text: "{raw_requirement}"
        Current JSON: {json.dumps(current_json)}
        USER FEEDBACK: "{user_feedback}"
        Apply the feedback to improve the JSON. Return the ENTIRE updated suite.
        """
        return self._call_model(prompt)
...
with tab1:
    req_input = st.text_area(
        ...
        "Example:\nThe payment system processes transactions. The transaction amount must be greater than $0.00 and cannot exceed $10,000.00. The credit card number must be exactly 16 digits.",
        height=100
    )
